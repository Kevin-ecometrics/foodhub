      ...
      {showCancelModal && (
        <div onClick={() => { setShowCancelModal(false); onCancelModalChange?.(false); }} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:800,padding:16,animation:"wr-fadein 0.2s ease" }}>
          <div onClick={e => e.stopPropagation()} style={{ background:"white",borderRadius:16,padding:"28px",maxWidth:340,width:"90%",boxShadow:"0 20px 60px rgba(0,0,0,0.2)",animation:"wr-scalein 0.22s ease" }}>
            ...
            <p style={{ fontSize:17,fontWeight:800,color:"var(--text)",marginBottom:6 }}>Cancelar producto</p>
            <p style={{ fontSize:13,color:"var(--muted)",marginBottom:16,lineHeight:1.5 }}>
              <strong style={{ color:"var(--text)" }}>{item.product_name}</strong>
              {availableToCancel > 1 && <span> · {availableToCancel} disponibles{cancelledQty > 0 ? ` (${cancelledQty} ya cancelados)` : ""}</span>}
            </p>
            ...
            {availableToCancel > 1 && (
              <>
                ...
                <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:20 }}>
                  <button onClick={() => setCancelQuantity(q => Math.max(1, q-1))} disabled={cancelQuantity<=1} style={{ width:38,height:38,borderRadius:10,border:"1.5px solid var(--border)",background:"white",cursor:"pointer",fontSize:20,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--text)",opacity:cancelQuantity<=1?0.35:1,fontFamily:"inherit" }}>−</button>
                  <span style={{ fontSize:22,fontWeight:800,color:"var(--text)",width:36,textAlign:"center" }}>{cancelQuantity}</span>
                  <button onClick={() => setCancelQuantity(q => Math.min(availableToCancel, q+1))} disabled={cancelQuantity>=availableToCancel} style={{ width:38,height:38,borderRadius:10,border:"1.5px solid var(--border)",background:"white",cursor:"pointer",fontSize:20,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--text)",opacity:cancelQuantity>=availableToCancel?0.35:1,fontFamily:"inherit" }}>+</button>
                </div>
              </>
            )}
            ...
            <div style={{ display:"flex",gap:10 }}>
              ...
              <button onClick={handleConfirmCancel} style={{ flex:1,padding:"11px 0",borderRadius:10,border:"none",background:"var(--red)",fontSize:13,fontWeight:700,color:"white",cursor:"pointer",fontFamily:"inherit" }}>
                {availableToCancel === 1 ? "Sí, cancelar" : `Cancelar ${cancelQuantity}`}
              </button>
            </div>
          </div>
        </div>
